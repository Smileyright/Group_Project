        ...
        <div className={styles.columns}>
          <div className={styles.column}>
            {leftColumn.map((art, i) => (
              <div
                key={art.objectID}
                className={`${styles.card} ${i % 2 === 0 ? styles.tallCard : styles.shortCard}`}
                onClick={() => setSelectedArt(art)}
              >
                <img src={art.primaryImageSmall} alt={art.title} className={styles.cardImage} />
          ...
          <div className={styles.column}>
            {rightColumn.map((art, i) => (
              <div
                key={art.objectID}
                className={`${styles.card} ${i % 2 === 0 ? styles.shortCard : styles.tallCard}`}
                onClick={() => setSelectedArt(art)}
              >
                <img src={art.primaryImageSmall} alt={art.title} className={styles.cardImage} />
                <h2 className={styles.cardTitle}>{art.title}</h2>
      ...
      {selectedArt && (
        <div className={styles.modalOverlay} onClick={() => setSelectedArt(null)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <button className={styles.closeButton} onClick={() => setSelectedArt(null)}>✕</button>
            <img src={selectedArt.primaryImage} alt={selectedArt.title} className={styles.modalImage} />
